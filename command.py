...
updater = Updater(token='684831500:AAGyBy1pNuOjnpOeTTPPeeAkc4Bm5H5BA_o')
	...
	if user is None:
		...
		if target == machine:
			...
			if balance < amount:
				...
				bot.send_message(chat_id=update.message.chat_id, text="@{0} tipped @{1} of {2} Quan".format(user, target, amount))
		else:
			bot.send_message(chat_id=update.message.chat_id, text="Error that user is not applicable.")

def balance(bot,update):
	quote_page = requests.get('https://www.worldcoinindex.com/coin/Quantis')
	...
	if user is None:
		...
		bot.send_message(chat_id=update.message.chat_id, text="@{0} your current balance is: {1} Quan ≈  ${2}".format(user,balance,fiat_balance))
...
def price(bot,update):
	...
	quote_page = requests.get('https://bittrex.com/api/v1.1/public/getticker?market=btc-Quan')
	soup = BeautifulSoup(quote_page.content, 'html.parser').text
	btc = soup[80:]
	sats = btc[:-2]
	bot.send_message(chat_id=update.message.chat_id, text="QuanTis is valued at {0} Δ {1} ≈ {2}".format(price,percent,sats) + " ฿")
...
def withdraw(bot,update):
	...
	if user is None:
		...
		if balance < amount:
			...
			bot.send_message(chat_id=update.message.chat_id, text="@{0} has successfully withdrew to address: {1} of {2} Quan" .format(user,address,amount))
...
def marketcap(bot,update):
	...
	name = name_box.text.replace("\n","")
	mc = re.sub(r'\n\s*\n', r'\n\n', name.strip(), flags=re.M)
	bot.send_message(chat_id=update.message.chat_id, text="The current market cap of Quantis is valued at {0}".format(mc))
